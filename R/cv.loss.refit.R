cv.loss.refit <- function(surv, predict.x, fit,method=c("square", "absolute", "huber", "tukey"), k_h=1.345, k_t=4.685, sigma=1){

  nobs <- dim(predict.x)[1]
  eta <- surv[,1] - predict.x %*% fit$beta_refit[-1]-fit$beta_refit[1]
  ...
  addy <- function(t){
    res <- rep(x.order[t[1],,drop = FALSE] %*% fit$beta_refit[-1]+fit$beta_refit[1], t[2]) + eta.order[((t[1]+1):nobs)[state.order[(t[1]+1):nobs]==1]]
    res
  }
  ...
  if (method == "square"){
    loss <- sum((newy-newx %*% fit$beta_refit[-1]-fit$beta_refit[1])^2*newweight)
  } else if (method == "absolute"){
    loss <- sum(abs(newy-newx %*% fit$beta_refit[-1]-fit$beta_refit[1])*newweight)
  } else if (method == "huber"){
    residuals <- newy-newx %*% fit$beta_refit[-1]-fit$beta_refit[1]
